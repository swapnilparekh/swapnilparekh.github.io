...
This is a reasonable assumption about one class of backdoor attack. It is not a valid assumption about the one we describe in ThoughtSteer (published as *Thinking Wrong in Silence: Backdoor Attacks on Continuous Latent Reasoning*).

## The New Attack Surface: Latent Reasoning

A growing class of models — exemplified by architectures that perform "thinking" in a compressed latent space before generating visible tokens — does not produce explicit reasoning that humans can inspect. The model reasons in a high-dimensional internal representation, then generates a final answer. The intermediate computation is not text; it is geometry.

This is an appealing design from a capability perspective. Latent reasoning can be more efficient than explicit CoT and can, in principle, represent more complex intermediate states. But it creates a new attack surface that has received almost no attention.
...
The key insight is Neural Collapse. During training, representations of same-class inputs converge to tight geometric clusters in representation space — a property called neural collapse. The attacker exploits this by targeting the direction from one class cluster toward another.
...
The disturbing part: individual latent vectors during the reasoning process often still contain information about the correct answer. The adversarial signal is not in any single token representation; it is in the collective trajectory. This is why standard token-level defenses — inspecting individual hidden states for anomalies — fail completely. There is no anomaly to find in any one place.
...
This is not a reason to abandon latent reasoning. It is a reason to develop safety techniques that operate on reasoning trajectories rather than individual token representations. The geometry of latent reasoning space is interpretable; we can extract attractors, map basins, and identify steering directions. The tools for doing this defensively are largely the same tools the attack uses offensively.

The adversarial and interpretability research communities are working on the same object from different sides. A backdoor that operates through neural collapse geometry is, from another angle, a finding about how latent reasoning models represent and process information. That convergence is worth taking seriously.
