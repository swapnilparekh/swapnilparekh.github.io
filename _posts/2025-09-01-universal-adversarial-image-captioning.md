...
Input-specific adversarial attacks on image models have been known since 2014. The adversarial machine learning literature has extensively studied how to perturb a given image to fool a classifier. These attacks are powerful but narrow: each attack is engineered for a specific input, and it does not transfer to other images without significant effort.

Universal attacks are different. A universal perturbation is input-agnostic: it is computed once and applied uniformly to any input. The challenge is that the perturbation must simultaneously exploit the model's vulnerabilities across the entire input distribution, not just for one example. Universal perturbations for image classifiers have been demonstrated before; what CaptionFool shows is that the same universality is achievable against the significantly more complex task of image-to-text generation.
...
**The gap between human and model perception.** The deepest implication of universal adversarial attacks is not any specific exploit — it is what they reveal about the gap between how models process images and how humans do. Humans are not fooled by these perturbations; we see the original image correctly. The perturbation is exploiting computational structure that is entirely absent from human visual processing. Models that are deployed as if they perceive the world the way humans do are carrying a systematic vulnerability that this gap exposes.
